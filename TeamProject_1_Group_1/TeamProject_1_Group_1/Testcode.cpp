...
try {
	...
	*/ 
	...
	t1.addAttribute("new");
	if(t1.size() != size){
		std::cout<< "Error: adding attribute already in Table changes Table\n";
	}
	...
	t1.addRecord(r8);
	if(t1.minimum("new") != "1"){
		std::cout<< "Error in t1.minimum() function\n";
	}
	if(t1.maximum("new") != "3"){
		std::cout<< "Error in t1.maximum() function\n";
	}
	//Another thought:
...
//	std::cout<< db.getTables();
	...
	if(t1.getSize() != 3){
		std::cout<< "Error adding attributes\n";
	}
