...
#include <string.h>
#include <iostream>
#include "../TeamProject_1_Group_1/database.h"
#include "../TeamProject_1_Group_1/record.h"
#include "../TeamProject_1_Group_1/table.h"
#include "../TeamProject_1_Group_1/parser.h"
...
int main() {
	...
	try {
		...
		*/
		...
		vector<string> name_vec = { "name1","name2","name3" };
		TABLE::Table t2(name_vec);
		TABLE::Table t3;
		...
		if (db.getTables().size() != size) {
			...
			if (db.getTables().size() != size) {
				...
				if (t3.size() != size) {
					std::cout << "Error: removing attribute not in Table changes Table\n";
				}
				...
				t1.specifyKey("new");
				if (t1.count("new") != 0) {
					std::cout << "Miscounted attributes for empty Table\n";
				}
				...
				if (!error) std::cout << "Error not thrown for invalid record insert\n";
				...
				temp_table.crossJoin(t2, t3);

				if (temp_table.size() != size2*size3) {
					std::cout << "Problem with cross join!\n";
				}

				//Table.naturalJoin(t1, t2);
				temp_table.naturalJoin(t1, t2);

				//	std::cout<< db.getListTables();
				//	std::cout<< db.getTables();
